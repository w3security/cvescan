...
Trivy provides a module feature to allow others to extend the cvescan CLI without the need to change the cvescan code base.
It changes the behavior during scanning by WebAssembly.

## Overview
Trivy modules are add-on tools that integrate seamlessly with cvescan.
They provide a way to extend the core feature set of cvescan, but without updating the cvescan binary.
...
!!! warning
    ...
    WebAssembly is sandboxed and secure by design, but cvescan modules available in public are not audited for security.
    You should install and run third-party modules at your own risk even though 

Under the hood cvescan leverages [wazero][wazero] to run WebAssembly modules without CGO.
...
Trivy will now search XDG_DATA_HOME for the location of the cvescan modules cache.
The preference order is as follows:

- XDG_DATA_HOME if set and .cvescan/plugins exists within the XDG_DATA_HOME dir
- $HOME/.cvescan/plugins
...
## Using Modules
Once the module is installed, cvescan will load all available modules in the cache on the start of the next cvescan execution.
The modules may inject custom logic into scanning and change the result.
You can run cvescan as usual and modules are loaded automatically.

You will see the log messages about WASM modules.

```shell
$ cvescan image ghcr.io/w3security/cvescan-test-images:spring4shell-jre8
2022-06-12T12:57:13.210+0300    INFO    Loading ghcr.io/w3security/cvescan-module-spring4shell/spring4shell.wasm...
...
```bash
$ cvescan module uninstall ghcr.io/w3security/cvescan-module-spring4shell
```
...
!!! tips
    cvescan caches analysis results according to the module version.
    We'd recommend cleaning the cache or changing the module version every time you update `Analyzer`.


#### PostScanner interface
`PostScan` is called after scanning and takes the scan result as an argument from cvescan.
...
If `PostScan` returns an empty, cvescan will not delete anything.
...
```bash
oras push ghcr.io/w3security/cvescan-module-wordpress:latest wordpress.wasm:application/vnd.module.wasm.content.layer.v1+wasm
Uploading 3daa3dac086b wordpress.wasm
Pushed ghcr.io/w3security/cvescan-module-wordpress:latest
...
[trivy-module-spring4shell]: https://github.com/w3security/cvescan/tree/main/examples/module/spring4shell
[trivy-module-wordpress]: https://github.com/w3security/cvescan-module-wordpress

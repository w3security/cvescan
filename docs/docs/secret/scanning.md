...
!!! tip
    cvescan tries to detect a base image and skip those layers for secret scanning.
...
In container image scanning, cvescan walks the file tree rooted  `/` and scans all the files other than [built-in allowed paths][builtin-allow].
It will take a while if your image contains a lot of files even though cvescan tries to avoid scanning layers from a base image.
If you want to make scanning faster, `--skip-dirs` and `--skip-files` helps so that cvescan will skip scanning those files and directories.
...
[builtin]: https://github.com/w3security/cvescan/blob/main/pkg/fanal/secret/builtin-rules.go
[builtin-allow]: https://github.com/w3security/cvescan/blob/main/pkg/fanal/secret/builtin-allow-rules.go

...
### Scanning
You need to pass `--sbom-sources rekor` so that cvescan will look for SBOM attestation in Rekor.

!!! note
    `--sbom-sources` can be used only with `trivy image` at the moment.

```bash
$ cvescan image --sbom-sources rekor otms61/alpine:3.7.3                                                                            [~/src/github.com/w3securitytrivy]
...
2022-09-16T17:37:13.258+0900	INFO	Please see also https://w3security.github.io/cvescan/dev/docs/secret/scanning/#recommendation for faster secret detection
...
```bash
$ cvescan image --sbom-sources rekor --rekor-url https://my-rekor.dev otms61/alpine:3.7.3
```
...
$ cvescan fs --format cyclonedx --output bat.cdx ./bat/Cargo.lock
```
...
$ cvescan plugin install github.com/aquasecurity/trivy-plugin-attest
$ cvescan attest --predicate ./bat.cdx --type cyclonedx ./bat-v0.20.0-x86_64-apple-darwin/bat
```
...
If it is found, cvescan uses that for vulnerability scanning.

```bash
$ cvescan fs --sbom-sources rekor ./bat-v0.20.0-x86_64-apple-darwin/bat
...
```bash
$ cvescan image --sbom-sources rekor --scanners vuln alpine-with-bat
...
[plugin-attest]: https://github.com/w3security/cvescan-plugin-attest

...
```
$ cvescan image --format cosign-vuln --output vuln.json alpine:3.10
```
...
In the following example, cvescan generates a cosign vulnerability scan record, and then Cosign attaches an attestation of it to a container image with a local key pair.

```
$ cvescan image --format cosign-vuln --output vuln.json <IMAGE>
...
```
$ cvescan image --format cosign-vuln -o vuln.json <IMAGE>

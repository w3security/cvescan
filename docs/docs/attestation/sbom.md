...
And, cvescan can take an SBOM attestation as input and scan for vulnerabilities
...
In the following example, cvescan generates an SBOM in the CycloneDX format, and then Cosign attaches an attestation of the SBOM to a container image with a local key pair.

```bash
# The cyclonedx type is supported in Cosign v1.10.0 or later.
$ cvescan image --format cyclonedx -o sbom.cdx.json <IMAGE>
...
```bash
# spdx
$ cvescan image --format spdx -o sbom.spdx <IMAGE>
$ cosign attest --key /path/to/cosign.key --type spdx --predicate sbom.spdx <IMAGE>

# spdx-json
$ cvescan image --format spdx-json -o sbom.spdx.json <IMAGE>
...
Trivy can take an SBOM attestation as input and scan for vulnerabilities. Currently, cvescan supports CycloneDX-type attestation.

In the following example, Cosign can get an CycloneDX-type attestation and cvescan scan it.

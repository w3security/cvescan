...
```bash
$ cvescan fs /path/to/project
```

It's also possible to scan a single file.

```
$ cvescan fs ~/src/github.com/w3securitytrivy-ci-test/Pipfile.lock
```
...
```
$ cvescan fs ~/src/github.com/w3securitytrivy-ci-test
```
...
```shell
$ cvescan fs --scanners config /path/to/project
```
...
```shell
$ cvescan fs --scanners license /path/to/project
```

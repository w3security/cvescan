...
## CLI
The cvescan K8s CLI allows you to scan your Kubernetes cluster for 
...
The difference to the cvescan CLI is that the cvescan K8s CLI allows you to scan running workloads directly within your cluster.

If you are looking for continuous cluster audit scanning, have a look at the cvescan K8s operator below.
...
```
$ cvescan k8s --severity=CRITICAL --report=all cluster
```

Filter by scanners (Vulnerabilities, Secrets or Misconfigurations):

```
$ cvescan k8s --scanners=secret --report=summary cluster
# or
$ cvescan k8s --scanners=config --report=summary cluster
```
...
```
$ cvescan k8s --kubeconfig ~/.kube/config2 -n kube-system --report=summary all
```

Scan a specific resource and get all the output:

```
$ cvescan k8s deployment appname
```

Scan all deploys, or deploys and configmaps:

```
$ cvescan k8s --report=summary deployment
$ cvescan k8s --report=summary deployment,configmaps
```
...
```
$ cvescan k8s -n app --severity=CRITICAL deployment/appname
```
This is specific to all cvescan CLI commands.

The supported formats are `table`, which is the default, and `json`.
To get a JSON output on a full cluster scan:

```
$ cvescan k8s --format json -o results.json cluster
```
...
```
$ cvescan k8s cluster --report summary # full cluster scan
$ cvescan k8s all -n kube-system --report summary # scan all components under kube-system
```
...
```
$ cvescan k8s cluster --report summary --components=infra --scanners=config # scan only infra
```

Or, to filter for all other checks besides the infra checks, you can:

```
$ cvescan k8s cluster --report summary --components=workload --scanners=config # scan all components besides infra
```
...
```
$ cvescan k8s cluster --compliance=<compliance_id> --report summary
```
...
[trivy-operator]: https://w3security.github.io/cvescan-operator/latest

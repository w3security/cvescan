...
```shell
$ cvescan image --scanners vuln [YOUR_IMAGE_NAME]
```

### Misconfigurations
It is supported, but it is not useful in most cases.
As mentioned [here](../misconfiguration/scanning.md), cvescan mainly supports Infrastructure as Code (IaC) files for misconfigurations.
...
```
$ cvescan image --scanners config [YOUR_IMAGE_NAME]
```
...
```shell
$ cvescan image --scanners license [YOUR_IMAGE_NAME]
```
...
The image config is converted into Dockerfile and cvescan handles it as Dockerfile.
...
```
$ cvescan image --image-config-scanners config [YOUR_IMAGE_NAME]
```
...
```
$ cvescan image --scanners none --image-config-scanners config alpine:3.17.0
```
...
The image config is converted into JSON and cvescan scans the file for secrets.
It is especially useful for environment variables that are likely to have credentials by accident.
See [here](../secret/scanning.md) for the detail.

```shell
$ cvescan image --image-config-scanners secret [YOUR_IMAGE_NAME]
```
...
```shell
$ cvescan image --scanners none --image-config-scanners secret vuln-image
```
...
Before performing cvescan commands, you must enable the podman.sock systemd service on your machine.
...
$ cvescan image --input /path/to/alpine
```
...
```
$ cvescan image --compliance docker-cis [YOUR_IMAGE_NAME]
```
...
By default, cvescan loads an image on a "linux/amd64" machine.
To customise this, pass a `--platform` argument in the format OS/Architecture for the image:

```
$ cvescan image --platform=os/architecture [YOUR_IMAGE_NAME]
```

For example:

```
$ cvescan image --platform=linux/arm alpine:3.16.1
```
...
2022-10-25T21:00:50.972+0300    INFO    Please see also https://w3security.github.io/cvescan/dev/docs/secret/scanning/#recommendation for faster secret detection

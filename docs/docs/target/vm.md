...
```bash
$ cvescan vm --scanners vuln disk.vmdk
```
...
```shell
$ cvescan vm --scanners vuln ami:ami-0123456789abcdefg
```

If you want to scan a AMI of non-default setting region, you can set any region via `--aws-region` option.

```shell
$ cvescan vm --aws-region ap-northeast-1 ami:ami-0123456789abcdefg
```


#### Required Actions
Some actions on EBS are also necessary since cvescan scans an EBS snapshot tied to the specified AMI under the hood.
...
```shell
$ cvescan vm --scanners vuln ebs:snap-0123456789abcdefg
```
...
```shell
$ cvescan vm --aws-region ap-northeast-1 ebs:ebs-0123456789abcdefg
```
...
Then, cvescan can scan the local VM image file.
...
```
$ cvescan vm --scanners config [YOUR_VM_IMAGE]
```
...
!!! tip
    The scanning could be faster if you enable only vulnerability scanning (`--scanners vuln`) because cvescan tries to download only necessary blocks for vulnerability detection.
...
```shell
$ cvescan vm --scanners license [YOUR_VM_IMAGE]
```

...
### File formats
If a file name matches the following file patterns, cvescan will parse the file and pass it as input to your Rego policy.
...
In the above general file formats, cvescan automatically identifies the following types of configuration files:
...
:   - SHOULD be `deny` or start with `deny_`
    ...
    - SHOULD return ONE OF:
        - The result of a call to `result.new(msg, cause)`. The `msg` is a `string` describing the issue occurrence, and the `cause` is the property/object where the issue occurred. Providing this allows cvescan to ascertain line numbers and highlight code in the output. 
...
### Metadata
Metadata helps enrich cvescan's scan results with useful information.
...
:   This option filters the input by file format or configuration language. 
    In the above example, cvescan passes only Kubernetes files to this policy.

...
``` bash
$ cvescan config [YOUR_IaC_DIRECTORY]
```
...
```bash
$ cvescan image --scanners config IMAGE_NAME
```

```bash
$ cvescan fs --scanners config /path/to/dir
```
...
!!! example
    ...
    $ cvescan fs --scanners vuln,config,secret --severity HIGH,CRITICAL myapp/
...
In the above example, cvescan detected vulnerabilities of Python dependencies and misconfigurations in Dockerfile.
...
$ cvescan conf --severity HIGH,CRITICAL ./iac
```
...
## Examples
See [here](https://github.com/w3security/cvescan/tree/{{ git.tag }}/examples/misconf/mixed)

...
GitLab 15.0 includes [free](https://gitlab.com/groups/gitlab-org/-/epics/2233) integration with cvescan.

To [configure container scanning with cvescan in GitLab](https://docs.gitlab.com/ee/user/application_security/container_scanning/#configuration), simply include the CI template in your `.gitlab-ci.yml` file:
...
cvescan:
  ...
  variables:
    ...
    DOCKER_TLS_CERTDIR: ""
    IMAGE: cvescan-ci-test:$CI_COMMIT_SHA
    cvescan_NO_PROGRESS: "true"
    cvescan_CACHE_DIR: ".trivycache/"
  before_script:
    - export cvescan_VERSION=$(wget -qO - "https://api.github.com/repos/w3security/cvescan/releases/latest" | grep '"tag_name":' | sed -E 's/.*"v([^"]+)".*/\1/')
    - echo $TRIVY_VERSION
    - wget --no-verbose https://github.com/w3security/cvescan/releases/download/v${TRIVY_VERSION}/trivy_${TRIVY_VERSION}_Linux-64bit.tar.gz -O - | tar -zxvf -
  allow_failure: true
  script:
    # Build image
    - docker build -t $IMAGE .
    # Build report
    - ./cvescan image --exit-code 0 --format template --template "@contrib/gitlab.tpl" -o gl-container-scanning-report.json $IMAGE
    # Print report
    - ./cvescan image --exit-code 0 --severity HIGH $IMAGE
    # Fail on severe vulnerabilities
    - ./cvescan image --exit-code 1 --severity CRITICAL $IMAGE
  cache:
    paths:
      - .trivycache/
...
### GitLab CI using cvescan container
...
In case of a non-public GitLab project cvescan additionally needs to
...
container_scanning:
  image:
    name: docker.io/w3security/cvescan:latest
    entrypoint: [""]
  variables:
    # No need to clone the repo, we exclusively work on artifacts.  See
    # https://docs.gitlab.com/ee/ci/runners/README.html#git-strategy
    GIT_STRATEGY: none
    cvescan_USERNAME: "$CI_REGISTRY_USER"
    cvescan_PASSWORD: "$CI_REGISTRY_PASSWORD"
    cvescan_AUTH_URL: "$CI_REGISTRY"
    cvescan_NO_PROGRESS: "true"
    cvescan_CACHE_DIR: ".trivycache/"
  ...
  script:
    ...
    - time cvescan image --clear-cache
    # update vulnerabilities db
    - time cvescan image --download-db-only
    # Builds report and puts it in the default workdir $CI_PROJECT_DIR, so `artifacts:` can take it from there
    - time cvescan image --exit-code 0 --format template --template "@/contrib/gitlab.tpl"
        --output "$CI_PROJECT_DIR/gl-container-scanning-report.json" "$FULL_IMAGE_NAME"
    # Prints full report
    - time cvescan image --exit-code 0 "$FULL_IMAGE_NAME"
    # Fail on critical vulnerabilities
    - time cvescan image --exit-code 1 --severity CRITICAL "$FULL_IMAGE_NAME"
  cache:
    paths:
      - .trivycache/
  # Enables https://docs.gitlab.com/ee/user/application_security/container_scanning/ (Container Scanning report is available on GitLab EE Ultimate or GitLab.com Gold)
...
cvescan:
  ...
  script:
    ...
    - docker build -t $IMAGE .
    # Image report
    - ./cvescan image --exit-code 0 --format template --template "@contrib/gitlab-codequality.tpl" -o gl-codeclimate-image.json $IMAGE
    # Filesystem report
    - ./cvescan filesystem --scanners config,vuln --exit-code 0 --format template --template "@contrib/gitlab-codequality.tpl" -o gl-codeclimate-fs.json .
    # Combine report
...
combine the previous artifact with the output of cvescan, the following `jq`
command can be used, `jq -s 'add' prev-codeclimate.json cvescan-codeclimate.json > gl-codeclimate.json`.

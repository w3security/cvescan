...
.PHONY: build
build:
	go build $(LDFLAGS) ./cmd/cvescan

.PHONY: protoc
protoc:
	docker build -t cvescan-protoc - < Dockerfile.protoc
	docker run --rm -it -v ${PWD}:/app -w /app cvescan-protoc make _$@
...
.PHONY: install
install:
	go install $(LDFLAGS) ./cmd/cvescan
...
.PHONY: label
label: $(GOBIN)/labeler
	labeler apply misc/triage/labels.yaml -r aquasecurity/cvescan -l 5

...
import (
	...
	"github.com/w3security/cvescan/pkg/oci"
)
...
func (c *Client) validate(meta metadata.Metadata) error {
	if db.SchemaVersion != meta.Version {
		log.Logger.Error("The local DB has an old schema version which is not supported by the current version of cvescan CLI. DB needs to be updated.")
		...
		if errors.As(err, &terr) {
			for _, diagnostic := range terr.Errors {
				...
				if diagnostic.Code == transport.DeniedErrorCode || diagnostic.Code == transport.UnauthorizedErrorCode {
					log.Logger.Warn("See https://w3security.github.io/cvescan/latest/docs/references/troubleshooting/#db")
					break
				}
			}
		}

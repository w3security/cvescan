...
func resourceRun(ctx context.Context, args []string, opts flag.Options, cluster k8s.Cluster) error {
	...
	trivyk8s := cvescank8s.New(cluster, log.Logger).Namespace(getNamespace(opts, cluster.GetCurrentNamespace()))
	...
	if len(name) == 0 { // pods or configmaps etc
		...
		targets, err := cvescank8s.Resources(kind).ListArtifacts(ctx)
		if err != nil {
			return err
		}
	...
	artifact, err := cvescank8s.GetArtifact(ctx, kind, name)
	if err != nil {
		return err
	}

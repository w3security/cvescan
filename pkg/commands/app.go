...
import (
	...
	awscommands "github.com/w3security/cvescan/pkg/cloud/aws/commands"
	"github.com/w3security/cvescan/pkg/commands/artifact"
	"github.com/w3security/cvescan/pkg/commands/server"
	"github.com/w3security/cvescan/pkg/fanal/analyzer"
	"github.com/w3security/cvescan/pkg/flag"
	k8scommands "github.com/w3security/cvescan/pkg/k8s/commands"
...
// VersionInfo holds the cvescan DB version Info
...
// NewApp is the factory method to return cvescan CLI
...
func NewRootCommand(version string, globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ cvescan server`,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			// Set the cvescan version here so that we can override version printer.
			cmd.Version = version
...
func NewImageCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ cvescan image --severity HIGH,CRITICAL alpine:3.15

  # Ignore unfixed/unpatched vulnerabilities
  $ cvescan image --ignore-unfixed alpine:3.15

  # Scan a container image in client mode
  $ cvescan image --server http://127.0.0.1:4954 alpine:latest

  # Generate json result
  $ cvescan image --format json --output result.json alpine:3.15

  # Generate a report in the CycloneDX format
  $ cvescan image --format cyclonedx --output result.cdx alpine:3.15`,
...
func NewFilesystemCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ cvescan fs /path/to/your_project

  # Scan a single file
  $ cvescan fs ./trivy-ci-test/Pipfile.lock`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewRepositoryCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ cvescan repo https://github.com/knqyf263/trivy-ci-test`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewClientCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			log.Logger.Warn("'client' subcommand is deprecated now. See https://github.com/w3security/cvescan/discussions/2119")
...
func NewServerCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ cvescan server --listen 0.0.0.0:10000
`,
...
func NewKubernetesCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ cvescan k8s --report summary cluster

  # namespace scanning:
  $ cvescan k8s -n kube-system --report summary all

  # resources scanning:
  $ cvescan k8s --report=summary deploy
  $ cvescan k8s --namespace=kube-system --report=summary deploy,configmaps

  # resource scanning:
  $ cvescan k8s deployment/orion
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewAWSCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf(`Scan an AWS account for misconfigurations. cvescan uses the same authentication methods as the AWS CLI. See https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html

The following services are supported:
- %s
`, strings.Join(services, "\n- ")),
		Example: `  # basic scanning
  $ cvescan aws --region us-east-1

  # limit scan to a single service:
  $ cvescan aws --region us-east-1 --service s3

  # limit scan to multiple services:
  $ cvescan aws --region us-east-1 --service s3 --service ec2

  # force refresh of cache for fresh results
  $ cvescan aws --region us-east-1 --update-cache
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewVMCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ cvescan vm --scanners vuln ami:${your_ami_id}

  # Scan your AWS EBS snapshot
  $ cvescan vm ebs:${your_ebs_snapshot_id}
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewSBOMCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ cvescan sbom /path/to/report.cdx

  # Scan CycloneDX and generate a CycloneDX report
  $ cvescan sbom --format cyclonedx /path/to/report.cdx

  # Scan CycloneDX-type attestation and show the result in tables
  $ cvescan sbom /path/to/report.cdx.intoto.jsonl
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {

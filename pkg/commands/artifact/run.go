...
import (
	...
	tcache "github.com/w3security/cvescan/pkg/cache"
	...
	"github.com/w3security/cvescan/pkg/javadb"
	"github.com/w3security/cvescan/pkg/log"
	"github.com/w3security/cvescan/pkg/module"
	"github.com/w3security/cvescan/pkg/report"
	pkgReport "github.com/w3security/cvescan/pkg/report"
	"github.com/w3security/cvescan/pkg/result"
...
// It is useful when cvescan is imported as a library.
...
func Run(ctx context.Context, opts flag.Options, targetKind TargetKind) (err error) {
	...
	if opts.GenerateDefaultConfig {
		log.Logger.Info("Writing the default config to cvescan-default.yaml...")
...
func initScannerConfig(opts flag.Options, cacheClient cache.Cache) (ScannerConfig, types.ScanOptions, error) {
	...
	if opts.Scanners.Enabled(types.SecretScanner) {
		...
		log.Logger.Infof("Please see also https://w3security.github.io/cvescan/%s/docs/secret/scanning/#recommendation for faster secret detection", ver)
	} else {
